...
df_year_s = train.groupby('year').mean(skipna=True)[['sales']]
df_year_s = df_year_s.reset_index()
df_year_s['color'] =['#e2e9d2', '#cbd1bd', '#b4baa8', '#9ea393', '#878b7e']

df_month_s = train.groupby('month').mean(skipna=True)[['sales']]
...
df_day_of_week_s = train.groupby('day_of_week').mean(skipna=True)[['sales']]
...
df_year = train.groupby('year').mmean(skipna=True)[['onpromotion']]
df_year = df_year.reset_index()
df_year['color'] =['#e2e9d2', '#cbd1bd', '#b4baa8', '#9ea393', '#878b7e']

df_month = train.groupby('month').mean(skipna=True)[['onpromotion']]
...
df_day_of_week = train.groupby('day_of_week').mean(skipna=True)[['onpromotion']]
